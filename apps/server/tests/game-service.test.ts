...
describe('GameService challenges', () => {
  it('freezes Party events and Custom turn duration in the authoritative game state', () => {
    ...
    const partyState = service.start(party);
    expect(partyState.tavernEvent).toMatchObject({ type: 'DRUNKEN', roundNumber: 1 });
    const custom = { ...room, code: 'CUS234', settings: { ...room.settings, gameMode: 'CUSTOM' as const, eventEnabled: false, turnDurationSeconds: 5, bulletCount: 3 } };
    expect(service.start(custom)).toMatchObject({ gameMode: 'CUSTOM', turnDurationSeconds: 5, tavernEvent: null });
  });
