...
手机浏览器优先的原创多人诈唬派对游戏。当前版本为 **V7.0**：保留 2–8 人服务器权威诈唬核心循环，并在默认关闭的房间开关后加入第一批低风险道具、公开酒馆事件与轻量角色能力。
...
真实多人联机测试在 `apps/server/tests/multiplayer.test.ts`，会启动临时服务并连接真实 Socket.IO 客户端，覆盖 8 人开局、断线恢复、三客户端 phaseSequence 同步、惩罚结果延迟公开，以及 V7 道具和角色能力开关链路。运行中的开发服务还可执行 `pnpm --filter @bluff-tavern/server exec tsx scripts/live-reconnect-v1.0.ts`，通过 Vite 的 Socket.IO 反代验证游戏内断线后恢复同一座位。
...
- Classic 和 Quick 仍是当前可开局模式；V7 扩展开关默认关闭，可单独启用道具、酒馆事件或角色能力。
...
详见 [`docs/V7_PLAN.md`](docs/V7_PLAN.md)、[`docs/V7.0-acceptance.md`](docs/V7.0-acceptance.md)、[`docs/GAME_RULES_V6.md`](docs/GAME_RULES_V6.md)、[`docs/GAME_STATE_MACHINE_V6.md`](docs/GAME_STATE_MACHINE_V6.md)、[`docs/GAME_PROTOCOL_V6.md`](docs/GAME_PROTOCOL_V6.md)、[`docs/GAME_ANIMATION_V6.md`](docs/GAME_ANIMATION_V6.md) 与 [`docs/V6.6-acceptance.md`](docs/V6.6-acceptance.md)。
